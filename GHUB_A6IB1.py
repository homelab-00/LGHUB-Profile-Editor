#!/usr/bin/env python3
# -------------------------------------------------------------
# Logitech G-Hub Profile Editor (Extended + Config)
# -------------------------------------------------------------
# Features:
# 1) Config-based DB path, so it's not hardcoded
# 2) First-run prompt if config is missing
# 3) "Settings" menu to change DB path at any time
# 4) Dark theme, Add/Delete entries, Icon usage, etc.
...
# Constants
# ---------------------------------------
CONFIG_FILENAME = "ghub_editor_config.json"  # Name of the config file
...
def get_config_path():
    """
    Returns the absolute path to the config file in the same folder as this script/exe.
    """
    exe_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(exe_dir, CONFIG_FILENAME)

def load_or_create_config():
    """
    Tries to load the config from 'ghub_editor_config.json'.
    If it doesn't exist, prompts the user to pick a DB path,
    then writes a new config file.
    Returns the loaded config dict with at least a "db_path" key.
    """
    ...
    # If we reach here, either config is missing or db_path is invalid.
...
def save_config(cfg):
    """
    Overwrites the entire config object to 'ghub_editor_config.json'.
    """
    config_path = get_config_path()
    try:
        with open(config_path, "w", encoding="utf-8") as f:
            json.dump(cfg, f, indent=2)
        logging.debug("Config updated/saved.")
    except Exception as e:
...
class GHubEditorApp:
    def __init__(self, master, config):
        self.master = master
        master.title("G-Hub Profile Editor (Config-Based)")
    ...
    def change_db_path(self):
        ...
        Updates config and reloads profiles.
        """
