#!/usr/bin/env python3
# -------------------------------------------------------------
# Logitech G-Hub Profile Editor (settings.db) w/ Icon Preview
# -------------------------------------------------------------
...
# 5) Shows a preview of the icon if the file is a valid BMP/PNG/ICO
# 6) On Save, writes the updated JSON back (BLOB) to DB
...
# Pillow for displaying images (BMP, PNG, ICO, etc.)
...
class GHubEditorApp:
    def __init__(self, master):
        ...
        # We'll keep a reference to the loaded icon so it doesn't get GC'd
        self.icon_tk = None
    ...
    def browse_icon(self):
        """Let user pick a new icon file, then set it in the icon_path_var and load preview."""
    ...
    def load_icon_preview(self):
        """
        Tries to open the file in self.icon_path_var and display a resized
        preview in self.icon_label using Pillow (ImageTk).
        If it fails or path is empty, sets the label text to "(No icon loaded)".
        """
        ...
        try:
            # Open the image
            img = Image.open(path)
            # Resize for a smaller preview (e.g. 64x64)
